...
public class ServerManager {
	...
	void destroyServer(){
		...
		foreach(GCPacketProcessor processor in mProcessorList){
	...
	public class GCPacketProcessor{
		...
		void processPacket(PacketData packet){
			...
			case GCconst.TYPE_RESOURCE:
				/*
				Debug.Log ("processPacket : TYPE_RESOURCE");
				//요청에 해당하는 파일을 읽어온다.
				string resourcePath = mResourceManager.getResourcePath(packet.code);
				//응답으로 보낼 패킷을 만든다.
				PacketData resourcePacket = new PacketData();
				resourcePacket.type = GCconst.TYPE_RESOURCE;
				resourcePacket.code = packet.code;
				resourcePacket.value = mResourceManager.getResourceSize(resourcePath);
				byte[] resourcePacketBuffer = getByteArray(resourcePacket);
				mSocket.Send(resourcePacketBuffer,resourcePacketBuffer.Length,0);
				//해당 파일을 보낸다.
				mSocket.SendFile(resourcePath);
				*/
				break;
			/**
		...
		void sendReousrces(){
			...
			for (ushort i=0; i<count; i++) {
				...
				buffer = getPacketByteArray(GCconst.TYPE_RESOURCE,i,size);
		...
		void sendResourceMap(){
			...
			string xml = new StreamReader (mResourceManager.getResourceMap ()).ReadToEnd();
